	...
	public override void OnInspectorGUI () {
        ...
        if (prefabGenerationRules)
        {
            ...
            for (int i = 0; i < PrefabRules.arraySize; i++)
            {
				...
				SerializedProperty preinstanciatedCount = prefabRule.FindPropertyRelative("PreinstanciatedCount");
                ...
                if (PrefabRulesExpanded[i])
                {
                    ...
                    if (!ruleDeleted)
                    {
						...
						EditorGUILayout.PropertyField(preinstanciatedCount, new GUIContent("Preinstanciated", "Number of instances created at start"));
